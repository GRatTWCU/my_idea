...
def convert_nuscenes_raw_to_eth(nuscenes_path: str, output_path: str):
    ...
    try:
        ...
        # nuScenesデータを読み込み
        # ここで nuscenes_path が v1.0-mini の親ディレクトリであることを確認
        ...
        # シーン数が少ない場合、分割比率を調整するか、ダミーデータでテストすることを検討
        if n_scenes < 10: # 例えば、シーンが少なすぎる場合は警告
            print(f"Warning: Only {n_scenes} scenes found. Data split might be too small for effective training/validation/testing.")
            # 例として、最低限の分割を行う
            train_end = int(n_scenes * 0.7) if n_scenes * 0.7 >= 1 else 1
            val_end = int(n_scenes * 0.9) if n_scenes * 0.9 >= train_end + 1 else train_end + 1
            if val_end >= n_scenes: # val_endがシーン数を超えないように調整
                val_end = n_scenes - 1 if n_scenes > 1 else n_scenes

            # train_end と val_end が有効な範囲内にあることを確認
            train_end = max(1, min(train_end, n_scenes - 2)) # 最低1シーン、テスト用に2シーン残す
            val_end = max(train_end + 1, min(val_end, n_scenes - 1)) # train_endより大きく、テスト用に1シーン残す
        ...
        def process_scenes(scenes: List, split_name: str):
            ...
            for scene_idx, scene in enumerate(scenes):
                ...
                if trajectories:   # 歩行者がいる場合のみ
                    ...
                    print(f"Skipped {split_name}/{scene_name}: No pedestrian trajectories found.")
        ...
        import traceback
        traceback.print_exc() # 詳細なエラーメッセージを表示
        return False
...
def main():
    ...
    # input_pathのデフォルト値を、GitHubからクローンしたデータセットの一般的なパスに修正
    # このパスは、v1.0-mini ディレクトリの「親ディレクトリ」を指す必要があります。
    # 例: プロジェクトルート/data/nuScenes_mini/nuScenes-panoptic-v1.0-mini
    parser.add_argument('--input_path', type=str, 
                        default='./data/nuScenes_mini/nuScenes-panoptic-v1.0-mini', # ここを修正しました！
                        help='Input path for nuScenes data (parent directory of v1.0-mini)')
    ...
    if args.mode == 'raw':
        ...
        # NuScenesクラスが期待するパスが存在するか事前にチェック
        expected_version_path = os.path.join(args.input_path, 'v1.0-mini')
        if not os.path.exists(expected_version_path):
            print(f"エラー: NuScenesデータセットのバージョンディレクトリ '{expected_version_path}' が見つかりません。")
            print(f"指定された --input_path '{args.input_path}' が正しいか確認してください。")
            print(f"このパスは 'v1.0-mini' ディレクトリの親ディレクトリである必要があります。")
            return # 処理を中断
