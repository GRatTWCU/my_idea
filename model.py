...
class SingularTrajectoryPredictor(nn.Module):
    ...
    def __init__(self, input_dim: int = 2, hidden_dim: int = 64, output_dim: int = 2,
        ...
        # エンコーダ出力次元調整
        self.encoder_projection = nn.Linear(hidden_dim * 2, hidden_dim)  # 双方向なので2倍
    ...
    def forward(self, input_traj: torch.Tensor, 
        ...
        for t in range(self.pred_len):
            ...
            # Teacher forcing（訓練時のみ）
            if training and hasattr(self, 'target_traj') and t < self.target_traj.shape[1] - 1:
                use_teacher_forcing = np.random.random() < self.teacher_forcing_ratio
                decoder_input = self.target_traj[:, t:t+1, :] if use_teacher_forcing else pred_step
            else:
    ...
    def contrastive_loss(self, features: torch.Tensor, 
            ...
            for i in range(num_peds):
                for j in range(i + 1, num_peds):
                    ...
                    if obstacle_positions is not None:
                        obs_pos = obstacle_positions[b]
                        # 簡単な距離ベースの判定（実際はより複雑な条件）
                        dist_diff = torch.norm(feat_i - feat_j)
                        if dist_diff < 1.0:  # 閾値
                            positive_pairs.append((feat_i, feat_j))
                        else:
                            negative_pairs.append((feat_i, feat_j))
        ...
        for pos_pair in positive_pairs:
            ...
            for neg_pair in negative_pairs:
                neg_anchor, negative = neg_pair
                if torch.equal(anchor, neg_anchor):
            ...
            if neg_sims:
                ...
                neg_exp = torch.exp(neg_sims / temperature).sum()
